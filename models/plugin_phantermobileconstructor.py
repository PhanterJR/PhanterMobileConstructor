...
db.define_table('plugin_phantermobileconstructor_keystore',
                Field('keytool', 'string',
                      label=CAT(T('keytool'), SPAN(" ?", _class="help",
                                                   _title=T("Program keytool from JavaDK."))),
                      requires=IS_NOT_EMPTY(),
                      ),
                Field('keyname', 'string', default='phantermobile',
                      label=CAT(T("keyname"), SPAN(
                          " ?", _class='help', _title=T("Name of keystore"))),
                      requires=IS_NOT_EMPTY(),
                      ),
                Field('aliasname', 'string', default='PhanterMobile',
                      label=CAT(T("aliasname"), SPAN(
                          " ?", _class='help', _title=T("Alias of keystore"))),
                      requires=IS_NOT_EMPTY(),
                      ),
                Field('validity', 'string', default="10000",
                      label=CAT(T("validity"), SPAN(" ?", _class='help', _title=T(
                          "Days to expirates the certifield"))),
                      requires=IS_NOT_EMPTY(),
                      ),
                Field('storepass', 'string',
                      label=CAT(T("storepass"), SPAN(
                          " ?", _class='help', _title=T("keystore password"))),
                      requires=IS_NOT_EMPTY(),
                      ),
                Field('keypass', 'string',
                      label=CAT(T("keypass"), SPAN(
                          " ?", _class='help', _title=T('key password'))),
                      requires=IS_NOT_EMPTY(),
                      ),
                Field('common_name', 'string', default='PhanterJR',
                      label=CAT(T("common_name"), SPAN(" ?", _class='help', _title=T(
                          "Adicional Infomation (eg. Your Name)"))),
                      requires=IS_NOT_EMPTY(),
                      ),
                Field('organization_unit', 'string', default="PhanterMobile",
                      label=CAT(T("organization_unit"), SPAN(
                          " ?", _class='help', _title=T("Organization unit (eg. PhanterMobile)"))),
                      requires=IS_NOT_EMPTY(),
                      ),
                Field('organization', 'string', default="Conexao Didata",
                      label=CAT(T("organization"), SPAN(" ?", _class='help', _title=T(
                          "Your Organization (eg. Conexão Didata)"))),
                      requires=IS_NOT_EMPTY(),
                      ),
                Field('district', 'string', default="Aracaju",
                      label=CAT(T("district"), SPAN(" ?", _class='help',
                                                    _title=T("Your District (eg. Aracaju)"))),
                      requires=IS_NOT_EMPTY(),
                      ),
                Field('yourstate', 'string', default="Sergipe",
                      label=CAT(T("state"), SPAN(" ?", _class='help',
                                                 _title=T("Your State (eg. Sergipe)"))),
                      requires=IS_NOT_EMPTY(),
                      ),
                Field('country', 'string', default="BR",
                      label=CAT(T("country"), SPAN(" ?", _class='help',
...
db.define_table('plugin_phantermobileconstructor_apps',
                ...
                Field('idapp', 'string',
                      ...
                      label=CAT(T('ID'), SPAN(" ?", _class="help", _title=T(
                ...
                Field('description', 'text',
                      ...
                      label=CAT(T('Description'), SPAN(" ?", _class="help", _title=T(
                          "Specifies metadata that may appear within app-store listings.")))
                      ),
                Field('apkversion', 'string',
                      default="0.0.1",
                      label=CAT(T('Version'), SPAN(" ?", _class="help", _title=T(
                          "Full version number expressed in major/minor/patch notation."))),
                      requires=IS_NOT_EMPTY()
                      ),
                Field('authorname', 'string',
                      default="PhanterMobile",
                      label=CAT(T('Name of the author'), SPAN(" ?", _class="help", _title=T(
                          "Name that may appear within app-store lisitngs."))),
                      requires=IS_NOT_EMPTY()
                      ),
                Field('authoremail', 'string',
                      default="contato@conexaodidata.com.br",
                      label=CAT(T('Email of the author'), SPAN(" ?", _class="help", _title=T(
                          "Email that may appear within app-store lisitngs."))),
                      requires=[IS_NOT_EMPTY(), IS_EMAIL()]
                      ),
                Field('authorwebsite', 'string',
                      default="http://conexaodidata.com.br/phantermobileconstructor",
                      label=CAT(T('Website of the author'), SPAN(" ?", _class="help", _title=T(
                          "Website that may appear within app-store lisitngs."))),
                      requires=IS_NOT_EMPTY()
                      ),
                ...
                Field('allowintent', 'list:string',
                      default=["http://*/*", "https://*/*",
                               "tel:*", "sms:*", "mailto:*", "geo:*"],
                      label=CAT(T('Allow-Intent'), SPAN(" ?", _class="help", _title=T(
...
db.define_table('plugin_phantermobileconstructor_apks',
                Field('application', 'reference plugin_phantermobileconstructor_apps', default=db(db.plugin_phantermobileconstructor_apps).select().first().id if not db(
                    db.plugin_phantermobileconstructor_apps).isempty() else '', requires=IS_IN_DB(db, db.plugin_phantermobileconstructor_apps, "%(appname)s - %(idapp)s - %(apkversion)s")),
                Field('apklevel', 'string', default='debug',
                      label=CAT(T('Build type'), SPAN(" ?", _class="help", _title=T(
                          "Perform a release build or Perform a debug build"))),
